...
class ProductsService {
  ...
  async addProduct({ title, body, tags }) {
    const id = nanoid(16);
  ...
  async editProductById(id, { title, body, tags }) {
    const updatedAt = new Date().toISOString();
    const query = {
      text: 'UPDATE products SET title = $1, body = $2, tags = $3, updated_at = $4 WHERE id = $5 RETURNING id',
